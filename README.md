...
在首次执行 `init` 前，请在项目根目录的 `novels/` 中放入至少 20 本你喜欢的完本小说 `.txt` 文件。程序不会下载整本小说，只读取这些本地文件来生成初始偏好画像。

`init` 会按章节边界切块，每本小说先由 LLM 总结人物、情节、关键词、爽点、雷点、文风和节奏，再把所有单书总结合成为初始用户偏好画像。已初始化过时，`init` 不会重复生成画像。
...
- 至少 20 本本地喜欢小说用于冷启动画像，保证反馈循环有稳定初始点。
